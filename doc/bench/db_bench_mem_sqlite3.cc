...
#ifdef SQLITE_ENABLE_MEMJOURNAL
static bool FLAGS_MEMJOURNAL_enabled = true;
#else
static bool FLAGS_MEMJOURNAL_enabled = false;
#endif

#ifdef SQLITE_DISABLE_JOURNAL
static bool FLAGS_JOURNAL_enabled = false;
#else
static bool FLAGS_JOURNAL_enabled = true;
#endif
...
class Benchmark {
 ...
 public:
  ...
  void Open() {
    ...
    if (FLAGS_MEMJOURNAL_enabled) {
      std::string WAL_stmt = "PRAGMA journal_mode = MEMORY";
      status = sqlite3_exec(db_, WAL_stmt.c_str(), NULL, NULL, &err_msg);
      ExecErrorCheck(status, err_msg);
    }

    if(!FLAGS_JOURNAL_enabled){
...
int main(int argc, char** argv) {
  ...
  sqlite3_config(SQLITE_CONFIG_MMAP_SIZE, 1024*1024*768, 1024*1024*766);
